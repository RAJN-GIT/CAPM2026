...
const {
  ...
  shouldUseCloudSdk
} = require('./utils/cloudSdkProvider')
...
class RemoteService extends cds.Service {
  init() {
    ...
    if (this.options.credentials) {
      ...
      if (shouldUseCloudSdk(this.destination)) {
        this.middlewares = {
    ...
    this.on('*', async function on_handler(req, next) {
      ...
      if (requestConfig.headers.accept?.match(/stream|image|pdf|tar/)) requestConfig.responseType = 'stream'
      else if (requestConfig.headers.accept?.match(/xml|html/)) requestConfig.responseType = 'text'
      ...
      const csrfConfig = requestConfig._autoBatchedGet ? this.csrfInBatch : this.csrf
      if (shouldUseCloudSdk(this.destination)) {
        // Mount resilience and csrf middlewares for SAP Cloud SDK
        requestConfig.middleware = [this.middlewares.timeout]
        if (csrfConfig && this.middlewares.csrf) requestConfig.middleware.push(this.middlewares.csrf)
      } else {
        // REVISIT: consider adding retry and circuit breaker support to native fetch client
        requestConfig.timeout = this.requestTimeout
        if (csrfConfig) requestConfig.csrf = csrfConfig
      }
  ...
  async handle(req) {
    if (this._requires_resolving(req)) {
      ...
      return this.dispatch(_req).then(result => postProcess(_req.query, result, this, true))
    }

    // we need to post process if alias was explicitly set in query
    const post_process = _isSelectWithAliasedColumns(req.query)
    return super.handle(req).then(result => (post_process ? postProcess(req.query, result, this, true) : result))
  }
}
